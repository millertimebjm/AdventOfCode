...
fn main() {
    ...
    for text in contents.lines() {
        ...
        if vent.FromX == vent.ToX 
            || vent.FromY == vent.ToY 
            || (vent.FromX.abs_diff(vent.ToX) == vent.FromY.abs_diff(vent.ToY)) {
            vents.push(vent);
        }
    }
    ...
    for vent in &vents {
        ...
        // println!("{}, {} -> {}, {}", vent.FromX, vent.FromY, vent.ToX, vent.ToY);
...
fn add_points(vent: &Vent, points: &mut [[i32; 1000]; 1000]) {
    ...
    } else if vent.FromX.abs_diff(vent.ToX) == vent.FromY.abs_diff(vent.ToY) {
        if vent.FromX < vent.ToX && vent.FromY < vent.ToY {
            for index in 0..(vent.ToX - vent.FromX + 1) {
                add_get_point(points, vent.FromX + index, vent.FromY + index);
            }
        }else if vent.FromX < vent.ToX && vent.FromY > vent.ToY {
            for index in 0..(vent.ToX - vent.FromX + 1) {
                add_get_point(points, vent.FromX + index, vent.FromY - index);
            }
        }else if vent.FromX > vent.ToX && vent.FromY < vent.ToY {
            for index in 0..(vent.FromX - vent.ToX + 1) {
                add_get_point(points, vent.ToX + index, vent.ToY - index);
            }
        } else if vent.FromX > vent.ToX && vent.FromY > vent.ToY {
            for index in 0..(vent.FromX - vent.ToX + 1) {
                add_get_point(points, vent.ToX + index, vent.ToY + index);
